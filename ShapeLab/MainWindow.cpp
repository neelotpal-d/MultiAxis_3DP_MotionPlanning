...
void MainWindow::createActions()
{
	...
	connect(ui->pushButton_reset, SIGNAL(released()), this, SLOT(clearScreen()));
	...
	connect(ui->pushButton_smooth, SIGNAL(released()), this, SLOT(smoothNormal()));
...
void MainWindow::runDHWcalculation() {
	...
	GcodeGene->initial(
		...
		ui->doubleSpinBox_cnc_r->value(), ui->doubleSpinBox_cnc_h->value()
	);
...
void MainWindow::runSingularityOpt() {
	...
	GcodeGene->verifyPosNor(true); // must open for update normal
	GcodeGene->testXYZBCE(false);
	viewAllWaypointLayers();
}
...
void MainWindow::runCollisionElimination() {
	...
	GcodeGene->testXYZBCE(true); // not output the inserted nodes
	viewAllWaypointLayers();
}



// next step: tianyu: need to opt 1. build a array set and insert more nodes 2. write Gcode 
...
void MainWindow::runGcodeSimulation() {
	...
	std::vector<std::string> CNCfileSet = { "B-axis", "C-axis", "Z-axis" ,"Y-axis","X-axis", "frame" };
	readCNCfile(CNCfileSet);
	viewAllWaypointLayers();
	// close the display of Slice for speed up the simulation
	PolygenMesh* Slices; PolygenMesh* extruderHead; PolygenMesh* platform;
	Slices = _detectPolygenMesh(LAYER, "Slices", false);
	if (Slices != NULL) {
		Slices->bShow = false;
	}
...
void MainWindow::readWayPointData(std::string packName) {
	...
	for (GLKPOSITION patchPos = waypointSet->GetMeshList().GetHeadPosition(); patchPos;) {
		...
		for (GLKPOSITION Pos = waypoint_patch->GetNodeList().GetHeadPosition(); Pos;) {
			...
			if (zz <= 0.0) Node->negativeZz = true; // 40.0 is the height of platform
...
void MainWindow::readExtruderHeadfile(std::string extruderHeadName) {
	...
	for (GLKPOSITION eHeadPos = eHead->GetNodeList().GetHeadPosition(); eHeadPos;) {
		...
		double xx, yy, zz;
		node->GetCoord3D(xx, yy, zz);
		//node->SetCoord3D(xx + Xmove, yy + Ymove, zz + Zmove);
		node->SetCoord3D(xx + Xmove, yy + Ymove, zz);
...
void MainWindow::readPlatformfile(std::string platformName) {
	...
	for (GLKPOSITION Pos = platformPatch->GetNodeList().GetHeadPosition(); Pos;) {
		...
		//Node->SetCoord3D(pp[0] - Xshift + Xmove, pp[1] - Yshift + Ymove, pp[2] - Zshift + Zmove);
		Node->SetCoord3D(pp[0] - Xshift + Xmove, pp[1] - Yshift + Ymove, pp[2] - Zshift);
		// record a base Position of node
		//Node->GetCoord3D(pp[0], pp[1], pp[2]);
...
void MainWindow::readCNCfile(std::vector<std::string> CNCfileSet) {
	...
	for (int i = 0; i < CNCfileSet.size(); i++) {
		sprintf(filename, "%s%s%s", "../DataSet/Sorce/CNCmodel/axis_", CNCfileSet[i].c_str(), ".obj");
		//cout << filename << endl;
		...
		for (GLKPOSITION Pos = cncPatch->GetNodeList().GetHeadPosition(); Pos;) {
			...
			double xx, yy, zz;
			node->GetCoord3D(xx, yy, zz);   node->SetCoord3D(xx, -zz, yy); // rotation_1
			node->GetCoord3D(xx, yy, zz);   node->SetCoord3D(-yy, xx, zz); // rotation_2
			node->GetCoord3D(xx, yy, zz);   node->SetCoord3D_last(xx, yy, zz); // set base position
			node->m_printPos << xx, yy, zz;

			if (CNCfileSet[i] == "B-axis")
				if (xx == 0.0 && yy == 0.0 && zz == 0.0)
					node->isPrintPnt = true;
		}
	}
...
void MainWindow::doTimerGcodeMoving(){
	...
	PolygenMesh* waypoints = _detectPolygenMesh(WAYPOINT, "Waypoints", false);
	PolygenMesh* cnc = _detectPolygenMesh(CNC_PARTS, "CNC_Model", false);
...
Eigen::Vector3d MainWindow::_calPartGuesture(Eigen::Vector3d printPos, double X, double Y, double Z, double B, double C) {
	...
	double r = ui->doubleSpinBox_cnc_r->value();
	double h = ui->doubleSpinBox_cnc_h->value();
	...
	temp_4d = Offset * Rot_C * Rot_B * Offset_back * part_Postion;
