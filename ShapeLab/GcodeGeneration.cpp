...
void GcodeGeneration::calDistance() {
	...
	double initial_largeJ_Length = 25.0;
	double support_largerJ_Length = 26.0;
		...
		for (int omptime = 0; omptime < Core; omptime++) {
			...
			for (GLKPOSITION Pos = polygenMesh_Waypoints->GetMeshList().GetHeadPosition(); Pos;) {
				...
				for (int Index = 0; Index < layerJumpPatchSet.size(); Index++) {
					...
					bool is_print = true;
					if (layerJumpPatchSet[Index]->GetNodeNumber() < toolpath_filter_threshold) is_print = false;
					...
					for (GLKPOSITION Pos = layerJumpPatchSet[Index]->GetNodeList().GetHeadPosition(); Pos;) {
						...
						if (is_print == false) { Node->is_print_node = false;    continue; }
						if (Node->Jump_SecIndex == (layerJumpPatchSet[Index]->GetNodeNumber() - 1)) continue;// ignore the last point(without next)
						...
						double delataC_deg = abs(Node->m_XYZBCE(4) - nextNode->m_XYZBCE(4));
...
void GcodeGeneration::verifyPosNor(bool checkSwitch) {
	...
	for (GLKPOSITION Pos = polygenMesh_Waypoints->GetMeshList().GetHeadPosition(); Pos;) {
		...
		for (GLKPOSITION Pos = WayPointPatch->GetNodeList().GetHeadPosition(); Pos;) {
			...
			double finalPx = X - r + r * cos(rad_C) - h * sin(rad_B) * sin(rad_C);
			double finalPy = Y + r * sin(rad_C) + h * sin(rad_B) * cos(rad_C);
...
void GcodeGeneration::writeGcode(std::string GcodeDir) {
	...
	double Z_home = 140;						// The hight of Home point; / mm
